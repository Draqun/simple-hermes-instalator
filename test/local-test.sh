...
full_test() {
  ...
  info "This downloads uv/Python/Node and clones the repos — expect several minutes."
  local out
  # systemd is not available in a plain container, so after our installer runs
  # (services will be skipped with a warning), we start the WebUI by hand via
  # ctl.sh and probe /health — proving the native stack actually runs on 2 GB.
  out="$(docker run --rm --memory=2g --memory-swap=2g --cpus=1 \
    -v "$REPO_DIR":"$MOUNT":ro -w "$MOUNT" -e NO_COLOR=1 "$IMAGE" \
    bash -c '
      set +e
      apt-get update -qq >/dev/null 2>&1
      apt-get install -y -qq curl git xz-utils ca-certificates >/dev/null 2>&1
      bash install-hermes-mikrus.sh --answers test/answers.example.env >/tmp/install.log 2>&1
      echo "___INSTALL_RC=$?"
      command -v hermes >/dev/null && echo "___HERMES=$(hermes --version 2>&1 | head -1)" || \
        { [ -x /usr/local/bin/hermes ] && echo "___HERMES=$(/usr/local/bin/hermes --version 2>&1 | head -1)"; }
      ls -d /usr/local/lib/hermes-agent /root/.hermes/hermes-agent 2>/dev/null | head -1 | sed "s/^/___AGENTDIR=/"
      ...
      if [ -f "$WUI/ctl.sh" ]; then bash "$WUI/ctl.sh" start >/tmp/webui.log 2>&1; fi
      for i in $(seq 1 40); do
        code=$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:8787/health 2>/dev/null)
